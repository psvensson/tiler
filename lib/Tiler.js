...
(function() {
  ...
  Tiler = (function() {
    ...
    Tiler.prototype.lookupZone = function(tileid, q) {
      var lruZone;
      if (debug) {
        console.log('Tiler.lookupZone called for ' + tileid);
      }
      lruZone = this.zones.get(tileid);
      if (lruZone) {
        if (debug) {
          console.log('Tiler.lookupZone resolving ' + tileid + ' from lru');
        }
        return q.resolve(lruZone);
      } else {
        this.zoneUnderConstruction[tileid] = true;
        return this.cacheEngine.get('Zone', tileid).then((function(_this) {
          return function(exists) {
            if (exists) {
              return _this.storageEngine.find('Zone', 'tileid', tileid).then(function(zoneObj) {
                if (debug) {
                  console.log('Tiler.lookupZone got back zone obj');
                }
                if (zoneObj) {
                  if (debug) {
                    console.log('Tiler.lookupZone resolving ' + tileid + ' from db');
                  }
    ...
    Tiler.prototype.resolveZoneFor = function(level, x, y) {
      ...
      if (debug) {
        console.log('Tiler.resolveZoneFor ' + tid + ' under construction = ' + underConstruction);
      }
