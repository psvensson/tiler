...
(function() {
  ...
  Tiler = (function() {
    ...
    Tiler.prototype.resolveZoneFor = function(level, x, y) {
      var cbs, lookupZone, q, registerZone, tid, underConstruction;
      q = defer();
      registerZone = (function(_this) {
        return function(q, zoneObj) {
          var arr, cbs, entityQT, itemQT, ztiles;
          ...
          zoneObj.items.forEach(function(item) {
            return _this.setSomething(level, item, itemQT, 'items', q, true).then(function(zo) {});
          });
          ...
          zoneObj.entities.forEach(function(entity) {
            return _this.setSomething(level, entity, entityQT, 'entities', q, true).then(function(zo) {});
          });
          ztiles = _this.zoneTiles[zoneObj.tileid] || {};
          zoneObj.tiles.forEach(function(tile) {
            return ztiles[tile.x + '_' + tile.y] = tile;
          });
          _this.zoneTiles[zoneObj.tileid] = ztiles;
          _this.zones.set(zoneObj.tileid, zoneObj);
      ...
      lookupZone = (function(_this) {
        return function(tileid, q) {
          var lruZone;
          lruZone = _this.zones.get(tileid);
          if (lruZone) {
            return q.resolve(lruZone);
          } else {
            _this.zoneUnderConstruction[tileid] = true;
            return _this.cacheEngine.get(tileid).then(function(exists) {
              if (exists) {
    ...
    Tiler.prototype.setSomething = function(level, something, qthash, propname, q, skipadd) {
      var qq;
      qq = defer();
