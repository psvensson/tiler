  ...
  Tiler = (function() {
    ...
    Tiler.prototype.getItemAt = function(level, x, y) {
      ...
      if (!level || !x || !y) {
        ...
        itemQT = this.zoneItemQuadTrees[tileid];
        this.getSomething(level, x, y, itemQT, q);
      }
      return q;
    };

    Tiler.prototype.getEntityAt = function(level, x, y) {
      var entityQT, q, tileid;
      q = defer();
      if (!level || !x || !y) {
        q.reject('Tiler.getTileAt got wrong parameters ');
      } else {
        tileid = this.getZoneIdFor(level, x, y);
        entityQT = this.zoneEntityQuadTrees[tileid];
        this.getSomething(level, x, y, entityQT, q);
      }
      return q;
    };

    Tiler.prototype.getSomething = function(level, x, y, qt, q) {
