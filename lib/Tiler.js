...
(function() {
  ...
  Tiler = (function() {
    function Tiler(storageEngine, cacheEngine, modelEngine, myAddress, communicationManager) {
      ...
      this.zoneTiles = {};
      this.zmgr = new ZonesManager(this);
      ...
      return this.zmgr.resolveZoneFor(level, x, y).then((function(_this) {
        return function(zoneObj) {
          ...
          if (zoneObj) {
            ...
            if (debug) {
              console.log('_getSomething for ' + level + ',' + x + ',' + y + ' got back');
            }
            if (debug) {
              console.dir(something);
            }
            if (something && something[0]) {
              return q.resolve(something[0].data);
            } else {
              return q.resolve();
            }
          }
      ...
      this.zmgr.resolveZoneFor(level, something.x, something.y).then((function(_this) {
        return function(zoneObj) {
          ...
          if (zoneObj) {
            qt = qthash[zoneObj.tileid];
            if (debug) {
              'Tiles._setSomething inserting ' + something.type + ' to zone ' + zoneObj.tileid;
            }
            console.dir(something);
            if (!something.width) {
              something.width = 1;
            ...
            if (debug) {
              console.log('=============== Tiler._setSomething resolving true for ' + something.type + ' at ' + zoneObj.tileid);
            }
            q.resolve(true);
    ...
    Tiler.prototype.removeSomething = function(level, something, qthash, propname, q) {
      ...
      this.zmgr.resolveZoneFor(level, something.x, something.y).then((function(_this) {
        return function(zoneObj) {
          var i, index, j, len, qt, rres, stuff, what;
          if (zoneObj) {
            qt = qthash[zoneObj.tileid];
