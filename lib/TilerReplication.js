...
(function() {
  ...
  TilerReplication = (function() {
    ...
    TilerReplication.TIME_BETWEEN_MASTER_PINGS = 2000;
    ...
    function TilerReplication(myAddress, cacheEngine, communicationManager) {
      ...
      this.oplogs = {};
      this.timers = {};
      this.replica_type = 'copy';
      this.imalivetimer = void 0;
    }
    ...
    TilerReplication.prototype.shutdown = function(zoneObj) {
      ...
      return this.cacheEngine["delete"]('zonereplica_' + zoneObj.tileid + ':' + this.myAddress);
    };
    ...
    TilerReplication.prototype.setOurselvesAsReplica = function(zoneObj, kind) {
      ...
      this.cacheEngine.set('zonereplica_' + zoneObj.tileid + ':' + this.myAddress, this.myAddress + "," + Date.now() + "," + kind);
      this.cacheEngine.expireat('zonereplica_' + zoneObj.tileid + ':' + this.myAddress, this.REPLICA_REGISTRATION_EXPIRATION);
      if (!this.imalivetimer) {
        return this.imalivetimer = this.registerTimer(this.imalive, zoneObj, TilerReplication.TIME_BETWEEN_IMALIVE, 'imalive_for_' + zoneObj.id);
      }
    };

    TilerReplication.prototype.imalive = function(zoneObj) {
      return this.setOurselvesAsReplica(zoneObj, this.replica_type);
    };
      ...
      this.getSiblingsForZone(zoneObj).then((function(_this) {
        return function(siblings) {
          ...
          if (siblings.length === 0) {
            ...
            if (!master) {
              return _this.registerOurselvesAsMasterFor(zoneObj, siblings).then(function() {
                return q.resolve(true);
              });
            } else {
    ...
    TilerReplication.prototype.registerOurselvesAsMasterFor = function(zoneObj, siblings) {
      ...
      if (siblings.length < 2 || this.weAreOldestReplicaFor(zoneObj, siblings)) {
        ...
        try {
          this.deRegisterTimer(zoneObj.id);
          this.replica_type = 'master';
          this.setOurselvesAsReplica(zoneObj, this.replica_type);
          return this.registerTimer(this.saveZone, zoneObj, TilerReplication.TIME_BETWEEN_MASTER_SAVES, 'master_saves_for_' + zoneObj.id);
        } catch (_error) {
          ex = _error;
